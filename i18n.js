...
const translations = {
    ja: {
        ...
        'sample_ml.method_proc': 'MATLAB上でNoRMCorreによる動き補正、およびCNMF（2光子）によるROI抽出を行いました。輝度変化(ΔF/F)の算出には、局所的なバックグラウンド減算とパーセンタイルベースのF₀推定を適用しています。',
            // AWS CLI Guide
        'cli.eyebrow': 'ダウンロードガイド',
        'cli.h1': 'BraiDyn-BCデータセットのダウンロードとアクセス',
        'cli.lead': 'BraiDyn-BCデータを手軽に取得できます。ローカルのコンピュータにダウンロードして探索するか、AWS EC2経由で接続してクラウド上の大量のデータを処理するかを選択してください。',
        'cli.btn_local': 'ローカルにダウンロード',
        'cli.btn_ec2': 'AWS EC2でアクセス',
        'cli.browser_view': 'ブラウザで見る',
        'cli.open_s3': 'S3のインデックスページを開く',
        'cli.method_title': 'ダウンロード方法の選択',
        'cli.method_desc': '目的に応じて、ご自身のコンピュータにデータを直接ダウンロードするか、クラウド上で直接操作するかを選択できます。',
        'cli.local_title': 'ローカルへのダウンロード (AWS CLI)',
        'cli.local_desc': 'ご自身のPCにデータをダウンロードし、オフラインで探索やスクリプトのテストを行うのに最適です。',
        'cli.ec2_title': 'クラウドでのアクセス (AWS EC2)',
        'cli.ec2_desc': 'ローカルのストレージ容量を消費せずに、大量のデータを直接クラウド上で解析するのに最適です。',
        'cli.method1': '方法 1',
        'cli.step1_h2': 'ローカルマシンからAWS CLIを使ってダウンロードする',
        'cli.install_cli': 'AWS CLIをインストールする',
        'cli.install_desc': 'データセットをダウンロードするには、<strong>すべてのOS</strong>（Windows、macOS、Linux）でAWS CLIが必要です。お使いのOSを選択し、インストール手順に従ってください。',
        'cli.win_p1': 'Windowsでは公式のMSIインストーラーの利用を推奨します:',
        'cli.win_li1': '公式ダウンロードリンク (<a href="https://awscli.amazonaws.com/AWSCLIV2.msi" target="_blank" rel="noopener noreferrer">AWSCLIV2.msi</a>) からインストーラーをダウンロードします。',
        'cli.win_li2': 'ダウンロードした <code>.msi</code> ファイルを実行し、画面の指示に従います。',
        'cli.win_p2': 'コマンドラインでインストールする場合は、PowerShell（管理者権限）を使用します:',
        'cli.mac_p1': '公式のPKGインストーラを使ってインストールします:',
        'cli.mac_p2': 'または、<a href="https://brew.sh/" target="_blank" rel="noopener noreferrer">Homebrew</a>を使っている場合:',
        'cli.linux_p1': 'ダウンローダーを解凍し、インストーラを実行します:',
        'cli.linux_p2': '<strong>ARM (aarch64)</strong> の場合はこちらのURLを使用してください:',
        'cli.verify': 'インストール後、ターミナルを開き以下のコマンドで確認してください:',
        'cli.install_note': '詳細は、<a href="https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html" target="_blank" rel="noopener noreferrer">公式のAWS CLIインストールガイド</a>をご覧ください。',
        'cli.os_note_title': 'Note',
        'cli.os_note_desc': '以下の <code>aws</code> コマンドは、AWS CLI がインストールされていれば Windows（PowerShell）/ macOS / Linux のいずれでも基本的に同じように実行できます。<br>OSごとの差異があるのは主に<strong>インストール方法</strong>や<strong>ファイルパスの指定方法</strong>です。',
        'cli.list_bucket': 'バケットの内容をリストする',
        'cli.list_confirm': '以下のコマンドを使用して、データセットにアクセスできることを確認します:',
        'cli.choose_dl': 'ダウンロードする対象を選択する',
        'cli.choose_desc': 'ダウンロードしたいデータの種類を選択し、生成されたコマンドをコピーしてください:',
        'cli.sel_session': 'セッションの種類',
        'cli.sel_subject': '被験体',
        'cli.sel_filetype': 'ファイルタイプ',
        'cli.sel_session_task': 'タスク',
        'cli.sel_session_rest': '安静時',
        'cli.sel_session_stim': '感覚刺激',
        'cli.sel_session_all': '全てのセッション',
        'cli.sel_subject_spec': '特定の被験体',
        'cli.sel_subject_all': '全ての被験体',
        'cli.sel_filetype_nwb': 'NWBのみ',
        'cli.sel_filetype_all': '全てのファイル',
        'cli.subject_name': '被験体名',
        'cli.method2': '方法 2',
        'cli.step2_h2': 'AWS EC2から直接アクセスする',
        'cli.step2_intro': 'この手順は、データセットが非常に大きく、ご自身のコンピュータにダウンロードする代わりにAWS上で解析を行いたい場合に便利です。',
        'cli.connect_ec2': 'EC2インスタンスに接続する',
        'cli.connect_desc': 'EC2インスタンスを起動し、SSHまたはEC2 Instance Connectを使用して接続します。',
        'cli.confirm_cli': 'AWS CLIが利用可能か確認する',
        'cli.list_bucket2': 'S3バケット内のファイルをリストする',
        'cli.choose_desc2': 'EC2にダウンロードしたいデータの種類を選択し、生成されたコマンドをコピーしてください:',
        'cli.notes_title': '注意事項',
        'cli.storage_title': 'ストレージ容量',
        'cli.storage_desc': '大容量のダウンロードを開始する前に、ローカルマシンまたはEC2インスタンスに十分なディスク空き容量があることを確認してください。',
        'cli.costs_title': 'AWSのコストについて',
        'cli.costs_desc': 'ご自身のコンピュータへのダウンロードにはEC2は不要ですが、EC2インスタンスを使用する場合はAWSのアカウントに課金が発生する可能性があります。',
        'cli.stop_title': 'ダウンロードの停止',
        'cli.stop_desc': '実行中のダウンロードは <code>Ctrl + C</code> で停止でき、後で同じ <code>aws s3 sync</code> コマンドを使用して再開できます。',
        'cli.shutdown_title': 'EC2の停止',
        'cli.shutdown_desc': 'EC2をご利用の場合は、不要な課金を避けるため作業完了後にインスタンスを停止または削除してください。',
},
    ...
    en: {
        ...
        // AWS CLI Guide
        'cli.eyebrow': 'Download Guide',
        'cli.h1': 'Download & Access the BraiDyn-BC Dataset',
        'cli.lead': 'Get the BraiDyn-BC data your way. Easily download files to your local computer for quick exploration, or connect via AWS EC2 to process large amounts of data in the cloud.',
        'cli.btn_local': 'Download Locally',
        'cli.btn_ec2': 'Access on AWS EC2',
        'cli.browser_view': 'Browser View',
        'cli.open_s3': 'Open S3 index page',
        'cli.method_title': 'Choose Your Download Method',
        'cli.method_desc': 'Depending on your needs, you can either download the dataset to your own computer or work with it directly in the cloud.',
        'cli.local_title': 'Local Download (AWS CLI)',
        'cli.local_desc': 'Best for downloading the data to your personal computer to explore, test scripts, and work offline.',
        'cli.ec2_title': 'Cloud Access (AWS EC2)',
        'cli.ec2_desc': 'Best for analyzing massive amounts of data directly in the cloud without using up your local storage space.',
        'cli.method1': 'Method 1',
        'cli.step1_h2': 'Download from your local machine using AWS CLI',
        'cli.install_cli': 'Install the AWS CLI',
        'cli.install_desc': 'The AWS CLI is required on <strong>all operating systems</strong> (Windows, macOS, and Linux) to download the dataset. Select your OS below and follow the installation instructions.',
        'cli.win_p1': 'Using the official MSI installer is recommended on Windows:',
        'cli.win_li1': 'Download the installer from the <a href="https://awscli.amazonaws.com/AWSCLIV2.msi" target="_blank" rel="noopener noreferrer">official download link (AWSCLIV2.msi)</a>.',
        'cli.win_li2': 'Run the downloaded <code>.msi</code> file and follow the on-screen instructions.',
        'cli.win_p2': 'If installing via the command line, use PowerShell (Administrator):',
        'cli.mac_p1': 'Install using the official PKG installer:',
        'cli.mac_p2': 'Or, if you use <a href="https://brew.sh/" target="_blank" rel="noopener noreferrer">Homebrew</a>:',
        'cli.linux_p1': 'Download, unzip, and run the installer:',
        'cli.linux_p2': '<strong>ARM (aarch64)</strong> users should use this URL instead:',
        'cli.verify': 'After installation, open your terminal and verify with:',
        'cli.install_note': 'For more details, see the <a href="https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html" target="_blank" rel="noopener noreferrer">official AWS CLI installation guide</a>.',
        'cli.os_note_title': 'Note',
        'cli.os_note_desc': 'Once the AWS CLI is installed, the following <code>aws</code> commands can be run in essentially the same way across Windows (PowerShell), macOS, and Linux.<br>Platform-specific differences primarily involve <strong>installation methods</strong> and <strong>file path syntax</strong>.',
        'cli.list_bucket': 'List the bucket contents',
        'cli.list_confirm': 'Use this command to confirm that you can access the dataset:',
        'cli.choose_dl': 'Choose what to download',
        'cli.choose_desc': 'Select the type of data you want to download, then copy the generated command:',
        'cli.sel_session': 'Session type',
        'cli.sel_subject': 'Subject',
        'cli.sel_filetype': 'File type',
        'cli.sel_session_task': 'Task',
        'cli.sel_session_rest': 'Resting State',
        'cli.sel_session_stim': 'Sensory Stim',
        'cli.sel_session_all': 'All Sessions',
        'cli.sel_subject_spec': 'Specific Subject',
        'cli.sel_subject_all': 'All Subjects',
        'cli.sel_filetype_nwb': 'NWB Only',
        'cli.sel_filetype_all': 'All Files',
        'cli.subject_name': 'Subject name',
        'cli.method2': 'Method 2',
        'cli.step2_h2': 'Access the data directly from AWS EC2',
        'cli.step2_intro': 'This option is useful when the dataset is large and you want to analyze it on AWS instead of downloading it to your personal computer.',
        'cli.connect_ec2': 'Connect to your EC2 instance',
        'cli.connect_desc': 'Start an EC2 instance and connect to it using SSH or EC2 Instance Connect.',
        'cli.confirm_cli': 'Confirm AWS CLI is available',
        'cli.list_bucket2': 'List files in the S3 bucket',
        'cli.choose_desc2': 'Select the type of data you want to download to EC2, then copy the generated command:',
        'cli.notes_title': 'Notes',
        'cli.storage_title': 'Storage',
        'cli.storage_desc': 'Make sure your local machine or EC2 instance has enough disk space before starting a large download.',
        'cli.costs_title': 'AWS costs',
        'cli.costs_desc': 'Downloading on your own computer does not require EC2, but using an EC2 instance may incur AWS charges.',
        'cli.stop_title': 'Stopping downloads',
        'cli.stop_desc': 'You can stop a running download with <code>Ctrl + C</code> and resume later with the same <code>aws s3 sync</code> command.',
        'cli.shutdown_title': 'EC2 shutdown',
        'cli.shutdown_desc': 'If you use EC2, stop the instance after you are done to avoid unnecessary charges.',
